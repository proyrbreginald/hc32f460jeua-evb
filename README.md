...
- **用法**: shell 中输入 `soak` (时长取 `CFG_SOAK_MINUTES`, 默认 10 分钟)
  或 `soak 60` (60 分钟) / `soak 0` (直到按 ESC); 期间 shell 兼任监控器,
  每 `CFG_SOAK_REPORT_INTERVAL_MS` (默认 30s) 输出一行进度, 含每个压力
  线程的循环数 (可直观发现 0 循环的"饥饿"线程);
- **压力线程**: 同时启动 18 个不同优先级的线程持续轰炸:
  调度器 (双 CPU 计算线程, 抢占+时间片) / 信号量 / 互斥量 (双线程竞争 +
  整除性/单调性/最终值三重校验, 捕获丢失更新) / 事件 / 邮箱 / 消息队列 /
  堆 (随机分配+模式回读) / 线程创建回收 (defunct 无泄漏) / 内核定时器
  漂移 / 调度延迟 / Flash 擦写 (节流) / CRC (硬件 vs 软件参考) /
  CAN 内部回环 (可用时);
...
- **监控判定** (每秒):
  ...
  - **SRAM 奇偶/ECC**、堆用量峰值、线程数、各线程栈水位持续采样;
- **结束报告** (始终打印): 运行时长 / PASS·FAIL 结论 / 每个压力线程的
  循环数与错误数 (**失败时附带首次失败的位置与时刻**) / 0 循环线程告警 /
  堆基线→峰值→结束 (泄漏判定) / 线程数泄漏判定 / 互斥量最终值校验 /
  最长调度延迟 / 各线程栈峰值百分比;
- **推荐实践**: 正式部署验证时开启 `CFG_WDT_ENABLE=true` —— 若调度器
  彻底停滞, 硬件看门狗会复位系统并留下失败证据; 建议连续运行至少
  **8 小时** (如 `soak 480`) 或过夜 (如 `soak 720`), Flash 压力默认
  10s 一次擦写循环, 24 小时约 8.6k 次, 远低于片内 Flash 寿命上限;
- `CFG_SOAK_ENABLE = false` 时命令提示"未启用"; 失败日志由压力线程
  即时输出 (console 整行原子, 不与其他线程输出交错), 进度与汇总由
  监控器统一打印。
