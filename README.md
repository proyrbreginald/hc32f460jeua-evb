...
- **输出纪律**: 协议字节与控制台文本共用 UART。接收端对帧前字节一律
  ...
  数据流中不打印, 避免干扰主机端流式发送;
- **会话互斥**: 数据收发期间业务线程打印经控制台**静默丢弃** (sz/rz 以
  会话守卫标注), 其他线程的日志/提示符不会与协议字节交错 (这是偶发
  FCS 失败/重传的常见根因); 协议自身进度输出与 panic 诊断不受影响;
...
现场 `r7`，按 `EXC_RETURN.bit2` 选择 MSP 或 PSP；`EXC_RETURN.bit4` 区分
基本帧（SP 起 0x20 字节）与扩展帧（FP 上下文在基本帧之后，共 0x68 字节）。
读取前会验证 `EXC_RETURN` 编码、地址范围、
...
- **共享文件系统**: 快照文件系统的实例移入全局
  ...
  检测拒绝, 单核临界区保证一致性);
- **写放大与磨损**: 快照文件系统每次提交都重写整个快照并回收旧扇区,
  刷新间隔即提交频率 —— 持续打日志时 `CFG_LOG_FLUSH_MS=2s` 约
  5.4k~10.8k 次擦除/扇区/天 (数据手册表 3-56 最小 10k 次, 典型 100k),
  因此默认 30s (`CFG_LOG_FLUSH_MS=30000`, 约 360~720 次/扇区/天;
  `CFG_LOG_RING` 相应放大到 16KiB, 掉电丢失窗口 = 刷新间隔 + RAM 缓冲)。
  缓冲为空时零 Flash 写入, 空间不足时自动删除最旧日志文件后重试;
- **掉电窗口**: 非 `reboot` 路径的掉电/复位最多丢失最近一个刷新间隔 +
  RAM 缓冲内的日志 (ring 容量 `CFG_LOG_RING`, 默认 16KiB); panic/fault
